<?php foreach ($getFindPeople as $getFollower) : ?>
  <?php $following = rowCount("SELECT followers_id FROM socmed.followers WHERE followers.followers_id = {$ID['user_id']} AND 
   user_id = {$getFollower["user_id"]}") ? "<input class='follow-btn' type='submit' name='unfollow' value='unfollow'>" : "<input class='follow-btn' type='submit' name='follow' value='follow'>";
